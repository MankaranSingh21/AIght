...
related: ["agents", "rag", "fine-tuning", "in-context-learning", "chain-of-thought"]
prerequisites: ["in-context-learning"]
successors: ["chain-of-thought", "function-calling"]
exemplar_tools: ["chatgpt", "claude", "perplexity"]
key_fields: ["creative-writing-literature", "marketing-advertising", "education-teaching", "journalism-media", "software-engineering"]
misconceptions:
  - "Prompt engineering is a fad — the models will get smart enough."
  - "There's one correct way to prompt."
  - "A better prompt can make any task possible."
...
<MarginNote side="right">"Prompt engineering" sounds grand. In practice it's mostly: writing the thing you actually want, with more specificity than you think you need.</MarginNote>

<Glossary term="prompt">Prompt engineering</Glossary> is the practice of deliberately structuring what you give a model to reliably get the output you want. It sounds simple. It's mostly craft.

<ConceptDemo shape="playground" ariaLabel="Prompt engineering toggle demo">
...
**Order affects emphasis.** Information near the start and end of a long context tends to get more weight in the output. Burying your main constraint in the middle of a long prompt is genuinely risky — the model can just miss it.<Footnote>This is the "<Glossary term='lost-in-the-middle'>lost in the middle</Glossary>" effect — first measured rigorously by Liu et al. in 2023 and confirmed across nearly every model generation since.</Footnote>
...
<MarginNote side="right">The trick isn't being clever. It's being honest about what you actually want, in the level of detail you'd give a competent stranger.</MarginNote>
...
### <Glossary term="few-shot">Few-shot</Glossary>
...
### <Glossary term="chain-of-thought">Chain-of-thought</Glossary>
...
The "step by step" instruction isn't decoration. It changes what the model produces.<Cite source="Wei et al., Chain-of-Thought Prompting Elicits Reasoning in Large Language Models (2022)" href="https://arxiv.org/abs/2201.11903" /> By generating intermediate reasoning tokens, the model effectively gives itself more surface area to get the right answer — working the problem rather than pattern-matching to an answer shape.
...
### <Glossary term="system-prompt">System prompts</Glossary>
...
<MarginNote side="right">If you've ever found a system prompt leak online and gone "wait, *that's* what's behind this product?" — that's the whole game.</MarginNote>
...
<MarginNote side="right">A trick that almost always works: ask the model to think first, *then* output the final JSON inside specific tags like &lt;answer&gt;...&lt;/answer&gt;. Best of both worlds.</MarginNote>

---

## Where prompt engineering hits its limits

Prompt engineering can shape how a model uses its knowledge. It can't create knowledge the model doesn't have. It can improve consistency, but not reliability on tasks that require precise factual recall — for that, you need <Glossary term="rag">retrieval</Glossary> or <Glossary term="fine-tuning">fine-tuning</Glossary>.

It also doesn't age well. A prompt optimized for one model version may produce notably different results on a newer version of the same model.<Cite source="Anthropic, Prompt engineering overview" href="https://docs.anthropic.com/en/docs/build-with-claude/prompt-engineering/overview" /> Evaluating against a test set matters more than intuition about what "should" work.
